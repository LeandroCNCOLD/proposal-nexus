...
function AppLayout() {
  ...
  if (profile?.must_change_password) {
    const submitPasswordChange = async (event: React.FormEvent) => {
      ...
      const { error: profileUpdateError } = await supabase.from("profiles").update({ must_change_password: false }).eq("id", user.id);
      setChangingPassword(false);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-6">
        <form onSubmit={submitPasswordChange} className="w-full max-w-sm rounded-xl border bg-card p-6 shadow-[var(--shadow-sm)]">
          <h1 className="text-xl font-semibold tracking-tight">Troque sua senha</h1>
          <p className="mt-2 text-sm text-muted-foreground">Você recebeu uma senha provisória. Defina uma nova senha para continuar.</p>
          <div className="mt-6 space-y-4">
            <input className="h-10 w-full rounded-md border bg-background px-3 text-sm" type="password" autoComplete="new-password" placeholder="Nova senha" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} />
            <input className="h-10 w-full rounded-md border bg-background px-3 text-sm" type="password" autoComplete="new-password" placeholder="Confirmar nova senha" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} />
            {passwordError && <p className="text-sm text-destructive">{passwordError}</p>}
            <button className="inline-flex h-10 w-full items-center justify-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50" disabled={changingPassword} type="submit">
