...
function ProductDialog({ product, onOpenChange, onSave, saving }: { product: ProductForm | null; onOpenChange: (open: boolean) => void; onSave: (product: ProductForm) => void; saving: boolean }) {
  ...
  const num = (key: keyof ProductForm) => ({ value: typeof form[key] === "number" ? form[key] : "", onChange: (e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, [key]: e.target.value === "" ? null : Number(e.target.value) }) });
