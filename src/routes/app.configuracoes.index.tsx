...
import {
  APP_MODULES,
  MODULE_ACCESS_DESCRIPTION,
  roleCanAccessPath,
  type RoleModuleAccess,
} from "@/lib/module-access";
import {
  approveUserAccessQueueItem,
  nomusImportInternalUsersToAccessQueue,
...
function SettingsPage() {
  ...
  const [newUser, setNewUser] = useState({
    full_name: "",
    email: "",
    suggested_role: "coldpro" as AppRole,
  });
  const [accessSearch, setAccessSearch] = useState("");
  const [temporaryPasswords, setTemporaryPasswords] = useState<Record<string, string>>({});
  const [profileTemporaryPasswords, setProfileTemporaryPasswords] = useState<
    ...
    queryFn: async () =>
      (
        await supabase
          .from("profiles")
          .select(
            "id, full_name, email, access_status, access_source, nomus_user_id, blocked_reason, must_change_password",
          )
          .order("full_name")
      ).data ?? [],
  ...
  const rolesByUser = useMemo(() => {
    ...
    for (const item of userRoles)
      grouped.set(item.user_id, [...(grouped.get(item.user_id) ?? []), item.role]);
    return grouped;
  }, [userRoles]);
  ...
  const filteredAccessQueue = useMemo(() => {
    ...
    return accessQueue.filter((item) =>
      [item.full_name, item.email, item.source, item.nomus_user_id]
        .filter(Boolean)
  ...
  const filteredProfiles = useMemo(() => {
    ...
    return allProfiles.filter((item) =>
      [item.full_name, item.email, item.access_source, item.nomus_user_id]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(searchTerm)),
    );
  ...
  const updateProfileStatus = async (
    profileId: string,
    access_status: "active" | "blocked" | "pending",
  ) => {
    const { error } = await supabase
      .from("profiles")
      .update({
        access_status,
        blocked_reason: access_status === "blocked" ? "Bloqueado pelo gestor" : null,
      })
      .eq("id", profileId);
    if (error) return toast.error(error.message);
    refreshAccess();
  ...
  const toggleModuleAccess = async (
    ...
    const { error } = await supabase
      ...
      .upsert(
        { role, module_key: module.key, module_path: module.path, allowed },
  ...
  const updateQueueStatus = async (
    ...
    if (status === "approved") {
      ...
      if (temporaryPassword.length < 8)
        return toast.error("Informe uma senha provisória com pelo menos 8 caracteres.");
    ...
    refreshAccess();
    toast.success(
      status === "rejected" ? "Pré-liberação rejeitada." : "Pré-liberação voltou para pendente.",
    );
  };
  ...
  return (
    <>
      <PageHeader
        title="Configurações"
        subtitle="Perfis, módulos liberados e senhas provisórias dos usuários"
      />
          ...
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Ao marcar um perfil, o sistema libera automaticamente os módulos correspondentes
                para o usuário.
              </p>
            </div>
            <Badge variant={canManageAccess ? "default" : "outline"}>
              {canManageAccess ? "Gestor" : "Sem gestão"}
            </Badge>
          </div>
          {!canManageAccess ? (
            <div className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
              Seu perfil atual não permite liberar acessos.
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex flex-col gap-2 rounded-lg border bg-background/40 p-4 md:flex-row md:items-end md:justify-between">
                <div className="space-y-1.5 md:min-w-[340px]">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder="Nome, e-mail, origem ou ID Nomus"
                    />
                  </div>
                </div>
                <Button variant="outline" onClick={importNomusUsers} disabled={importingNomus}>
                  ...
                  {importingNomus
                    ? "Buscando usuários internos"
                    : "Buscar usuários internos no Nomus"}
                </Button>
              </div>
              ...
              <div className="grid gap-3 rounded-lg border bg-background/40 p-4 md:grid-cols-[1fr_1fr_180px_auto]">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setNewUser((value) => ({ ...value, full_name: event.target.value }))
                    }
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    value={newUser.suggested_role}
                    onChange={(event) =>
                      setNewUser((value) => ({
                        ...value,
                        suggested_role: event.target.value as AppRole,
                      }))
                    }
                  >
                ...
                {ACCESS_ROLES.map((role) => (
                  <div key={role} className="rounded-md border bg-card p-3">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {MODULE_ACCESS_DESCRIPTION[role]}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <h3 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                    <Clock3 className="h-3.5 w-3.5" /> Pendentes de liberação
                  </h3>
                      ...
                      filteredAccessQueue.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="text-right">
                            ...
                            >
                              {item.status === "approved" ? "Reabrir" : "Aprovar"}
                            </Button>
                          </TableCell>
                        </TableRow>
                      ))
              ...
              <div>
                <h3 className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  <ShieldCheck className="h-3.5 w-3.5" /> Usuários com login
                </h3>
                <Table>
                  <TableHeader>
                    <TableRow>
                        ...
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            <div className="flex flex-col items-start gap-1">
                              ...
                              >
                                {ACCESS_STATUS_LABELS[item.access_status] ?? item.access_status}
                              </Badge>
                              {item.must_change_password && (
                                <Badge variant="outline">Troca pendente</Badge>
                              )}
                            </div>
                          </TableCell>
                          <TableCell>
