  ...
  const { data: activity = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const notes = supabase.from('crm_notes')
        .select('id, body, created_at, created_by').eq('process_id', id)
        .order('created_at', { ascending: false }).limit(200)
      const fups = supabase.from('crm_followups')
        .select('id, due_at, kind, note, status, created_at, created_by').eq('process_id', id)
        .order('created_at', { ascending: false }).limit(100)
      const stages = supabase.from('crm_stage_changes')
        .select('id, from_etapa, to_etapa, created_at, created_by').eq('process_id', id)
        .order('created_at', { ascending: false }).limit(100)
