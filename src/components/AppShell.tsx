...
function AppNavigationSidebar() {
  ...
  const groups = NAV.map((group) => ({
    ...group,
    items: group.items.filter((item) => isAppRouteAllowed(item.to, roles, moduleAccess)),
  })).filter((group) => group.items.length > 0);

  return (
    <Sidebar
      collapsible="icon"
      className="app-sidebar border-sidebar-border bg-sidebar text-sidebar-foreground"
    >
      <SidebarHeader className="border-b border-sidebar-border p-2">
        <div className="flex h-9 items-center gap-2 px-1">
          ...
          <div className="min-w-0 leading-tight group-data-[collapsible=icon]:hidden">
            ...
            <div className="truncate text-[10px] uppercase tracking-widest text-sidebar-foreground/50">
              Sales Intelligence
            </div>
          </div>
        </div>
    ...
    <SidebarProvider>
      <div className="app-shell flex min-h-screen w-full bg-background">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="app-topbar sticky top-0 z-30 border-b bg-card/90 backdrop-blur">
            <div className="flex min-h-12 items-center gap-2 px-2 py-1 sm:px-3">
              ...
              <div className="relative hidden max-w-xl flex-1 sm:block">
                ...
                <Input
                  placeholder="Buscar propostas, clientes, equipamentos..."
                  className="compact-input h-8 border-transparent bg-secondary/50 pl-8 focus:bg-card"
                />
              </div>
                  ...
                  <button className="flex items-center gap-1.5 rounded-full py-0.5 pl-0.5 pr-1.5 hover:bg-secondary">
                    ...
                    <div className="hidden text-left md:block">
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        {primaryRole ? ROLE_LABELS[primaryRole] : "Sem perfil"}
                      </div>
                    </div>
          ...
          <main className="app-main min-w-0 flex-1 overflow-x-auto">{children}</main>
        </div>
      </div>
    </SidebarProvider>
  );
