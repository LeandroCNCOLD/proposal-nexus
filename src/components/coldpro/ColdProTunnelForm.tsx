...
export const ColdProTunnelForm = React.forwardRef<ColdProTunnelFormHandle, ColdProTunnelFormProps>(function ColdProTunnelForm({ environmentId, environment, product, tunnel, productCatalog = [], onSave }, ref) {
  ...
  const catalogThermalNum = (key: string) => ({ ...thermalNum(key), readOnly: Boolean(form.product_id), readOnlyValue: Boolean(form.product_id), title: form.product_id ? "Valor vindo do catálogo do produto" : undefined });
  ...
  const latentModeLabel = String(catalogKcalProduct?.latentMode ?? form.latent_mode ?? "effective") === "full" ? "total × fração" : "efetivo do catálogo";
  const latentEffectiveKcalKg = String(catalogKcalProduct?.latentMode ?? form.latent_mode ?? "effective") === "full" ? latentHeatKcalKg * Math.max(0, Math.min(frozenWaterFraction, 1)) : latentHeatKcalKg;
  ...
  const modelCycleDifferenceLabel = thermalTimeRequiredMin <= 0 || availableTimeForMassMin <= 0 ? "—" : retentionAdjustmentMin > 0 ? `${fmtColdPro(retentionAdjustmentMin, 1)} min acima do ciclo` : "dentro do ciclo";
  ...
  const cyclesPerHour = availableTimeForMassMin > 0 ? 60 / availableTimeForMassMin : 0;
    ...
    <div className="space-y-4">
      ...
      <div className={`rounded-lg border p-4 ${tunnelResult.status === "adequate" && manualHWM2K > 0 ? "bg-success/10 border-success/20" : "bg-warning/10 border-warning/20"}`}>
        <div className="text-sm font-semibold text-foreground">
          Capacidade horária do girofreezer: {fmtColdPro(nominalCapacityKgH, 1)} kg/h, equivalente a {fmtColdPro(capacityKgMin, 2)} kg/min.
        </div>
      ...
      <div className="grid grid-cols-1 gap-x-10 xl:grid-cols-2">
        ...
        <div>
          <div className="mb-3 grid gap-3 sm:grid-cols-2">
            ...
            <ColdProCalculatedInfo label="2. Capacidade atual por minuto" value={`${fmtColdPro(capacityKgMin, 2)} kg/min`} description="kg/h ÷ 60" tone={capacityKgMin > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="3. Retenção atual do ciclo" value={`${fmtColdPro(availableTimeForMassMin, 1)} min`} description={isStatic ? "tempo de batelada" : "tempo disponível"} tone={availableTimeForMassMin > 0 ? "info" : "warning"} />
            <ColdProCalculatedInfo label="4. Massa na retenção atual" value={`${fmtColdPro(projectedCycleMassKg, 1)} kg`} description="kg/min × retenção" tone={projectedCycleMassKg > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="5. Status do ciclo" value={hasPhysicalCompatibility ? "Compatível" : "Revisar premissas"} description="compatível com o congelamento" tone={hasPhysicalCompatibility ? "success" : "warning"} />
            <ColdProCalculatedInfo label="6. Retenção operacional do ciclo" value={availableTimeForMassMin > 0 ? `${fmtColdPro(availableTimeForMassMin, 1)} min` : "—"} description="tempo operacional informado" tone={availableTimeForMassMin > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="7. Tempo estimado pelo modelo térmico" value={thermalTimeRequiredMin > 0 ? `${fmtColdPro(thermalTimeRequiredMin, 1)} min` : "—"} description="auditoria física; não altera a capacidade" tone={thermalTimeRequiredMin > 0 && thermalTimeRequiredMin <= availableTimeForMassMin ? "success" : "warning"} />
            <ColdProCalculatedInfo label="8. Diferença modelo × ciclo" value={modelCycleDifferenceLabel} description="comparação, não ajuste automático" tone={retentionAdjustmentMin <= 0 && thermalTimeRequiredMin > 0 ? "success" : "warning"} />
            ...
            <ColdProCalculatedInfo label="10. Ciclos por hora" value={cyclesPerHour > 0 ? `${fmtColdPro(cyclesPerHour, 2)} ciclos/h` : "—"} description="60 ÷ retenção do ciclo" tone={cyclesPerHour > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="11. Capacidade atual por minuto" value={capacityKgMin > 0 ? `${fmtColdPro(capacityKgMin, 2)} kg/min` : "—"} description="capacidade nominal ÷ 60" tone={capacityKgMin > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="12. Capacidade atual por hora" value={nominalCapacityKgH > 0 ? `${fmtColdPro(nominalCapacityKgH, 1)} kg/h` : "—"} description="capacidade nominal do túnel" tone={nominalCapacityKgH > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="13. Temperatura do ar" value={`${fmtColdPro(airTemperatureC, 1)} °C`} description={airTempSource === "environment" ? "usando ambiente" : "túnel manual"} tone="info" />
            <ColdProCalculatedInfo label="14. Temperatura evaporação" value={`${fmtColdPro(evaporatorTempC, 1)} °C`} description="T_ar - ΔT" tone="info" />
            <ColdProCalculatedInfo label="15. Vazão necessária" value={fmtAirflow(requiredAirflowM3H)} description="carga kW × 3600 ÷ ρ × Cp × ΔT" tone={requiredAirflowM3H > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="16. Vazão informada" value={fmtAirflow(informedFanAirflowM3H)} description="ventiladores" tone={informedFanAirflowM3H > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="17. Velocidade do ar" value={`${fmtColdPro(tunnelResult.airVelocityUsedMS ?? displayedAirVelocityMS, 2)} m/s`} description="vazão ÷ seção livre" tone={(tunnelResult.airVelocityUsedMS ?? displayedAirVelocityMS) > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="18. h manual" value={manualHWM2K > 0 ? `${fmtColdPro(manualHWM2K, 2)} W/m²K` : "—"} description="obrigatório para validação final" tone={manualHWM2K > 0 ? "success" : "warning"} />
            <ColdProCalculatedInfo label="19. h sugerido" value={`${fmtColdPro(hSuggestedWM2K, 2)} W/m²K`} description="referência; não altera automático" tone={hSuggestedWM2K > 0 ? "info" : "warning"} />
          ...
          {showInsufficientTimeDiagnostic ? <div className="mb-3 rounded-lg border border-warning/20 bg-warning/10 p-3 text-sm text-warning"><div className="mb-1 flex items-center gap-2 font-semibold"><AlertTriangle className="h-4 w-4" /> Operacionalmente, o ciclo atual é de {fmtColdPro(availableTimeForMassMin, 1)} min para {fmtColdPro(projectedCycleMassKg, 1)} kg.</div><p>A capacidade nominal permanece {fmtColdPro(nominalCapacityKgH, 1)} kg/h, equivalente a {fmtColdPro(capacityKgMin, 2)} kg/min, com velocidade de esteira de {currentBeltSpeedMMin > 0 ? fmtColdPro(currentBeltSpeedMMin, 2) : "—"} m/min.</p><p className="mt-1">O modelo térmico estimou {fmtColdPro(thermalTimeRequiredMin, 1)} min até o núcleo; isso é uma auditoria física do produto e não uma previsão de produção por hora.</p><p className="mt-1 font-semibold">Com a operação atual, continuam passando {fmtColdPro(projectedCycleMassKg, 1)} kg a cada {fmtColdPro(availableTimeForMassMin, 1)} min, totalizando {fmtColdPro(nominalCapacityKgH, 1)} kg/h. Revise h, vazão, temperatura, espessura, geometria ou fator de penetração para tornar o tempo térmico compatível com a retenção.</p></div> : null}
        </div>
      </div>
      ...
      <div className="space-y-5">
          ...
          <ColdProFormSection title="Etapa 2 — Produto e geometria" description="Separe a dimensão da unidade/caixa da dimensão do pallet, carrinho ou bloco." icon={<Package className="h-4 w-4" />}>
            <div className="space-y-4"><div className="min-w-0 rounded-lg border bg-muted/20 p-3 sm:p-4"><div className="grid grid-cols-1 gap-x-8 gap-y-2 md:grid-cols-2 xl:grid-cols-4">
              ...
              <ColdProField label="Grupo do catálogo"><ColdProSelect value={selectedGroup} onChange={(e) => { setSelectedGroup(e.target.value); setProductSearch(""); set("product_id", null); }}><option value="">Seleção manual</option>{groups.map((group) => <option key={textValue(group)} value={textValue(group)}>{textValue(group)}</option>)}</ColdProSelect></ColdProField>
              <ColdProField label="Produto do catálogo"><ColdProSelect value={textValue(form.product_id)} disabled={filteredProducts.length === 0} onChange={(e) => applyProduct(e.target.value)}><option value="">{filteredProducts.length ? "Selecione o produto" : "Nenhum produto encontrado"}</option>{filteredProducts.map((p) => <option key={textValue(p.id)} value={textValue(p.id)}>{textValue(p.name)}</option>)}</ColdProSelect></ColdProField>
              ...
              {selectedCatalogProduct ? <div className="md:col-span-2 xl:col-span-4"><ColdProCalculatedInfo label="Dados do catálogo" value="Medidas e propriedades carregadas" description={textValue(selectedCatalogProduct.observations ?? selectedCatalogProduct.source_reference, "Produto do catálogo Ashworth")} tone="info" /></div> : null}
          ...
          <ColdProFormSection title="Etapa 4 — Temperaturas e propriedades térmicas" description={form.product_id ? "Propriedades técnicas carregadas do catálogo e bloqueadas para preservar a base oficial. Temperaturas de entrada/final e condições operacionais continuam editáveis." : "Temperaturas e propriedades manuais usadas na energia específica e na estimativa de tempo até o núcleo."} icon={<Thermometer className="h-4 w-4" />}>
            <div className="grid grid-cols-1 gap-x-10 xl:grid-cols-2"><div>
              <ColdProField label="Temp. entrada" helpKey="initialProductTemp" unit="°C"><ColdProInput {...num("inlet_temp_c")} /></ColdProField><ColdProField label="Temp. final" helpKey="finalProductTemp" unit="°C"><ColdProInput {...num("outlet_temp_c")} /></ColdProField><ColdProField label="Temp. congelamento" helpKey="freezingPoint" unit="°C"><ColdProInput {...catalogThermalNum("freezing_temp_c")} /></ColdProField><ColdProField label="Temperatura do ar" helpKey="airTemp" unit="°C"><ColdProInput {...num("air_temp_c")} /></ColdProField><ColdProField label="Fator penetração térmica" helpKey="thermalPenetrationFactor"><ColdProInput {...num("thermal_penetration_factor")} /></ColdProField>
            </div><div>
              <ColdProField label="Cp acima" helpKey="specificHeatAbove" unit="kcal/kg·°C"><ColdProInput {...catalogThermalNum("specific_heat_above_kcal_kg_c")} value={String(form.specific_heat_above_kcal_kg_c ?? "")} /></ColdProField><ColdProField label="Cp abaixo" helpKey="specificHeatBelow" unit="kcal/kg·°C"><ColdProInput {...catalogThermalNum("specific_heat_below_kcal_kg_c")} value={String(form.specific_heat_below_kcal_kg_c ?? "")} /></ColdProField><ColdProField label="Calor latente usado" helpKey="latentHeat" unit="kcal/kg"><ColdProInput {...catalogThermalNum("latent_heat_kcal_kg")} value={String(form.latent_heat_kcal_kg ?? "")} /></ColdProField><ColdProField label="Calor latente do catálogo" helpKey="latentHeat" unit="kJ/kg"><ColdProInput {...catalogThermalNum("latent_heat_kj_kg")} /></ColdProField><ColdProField label="Fração congelável" helpKey="frozenWaterFraction" unit="0–1"><ColdProInput {...catalogThermalNum("frozen_water_fraction")} /></ColdProField><ColdProField label="Densidade" helpKey="density" unit="kg/m³"><ColdProInput {...catalogThermalNum("density_kg_m3")} /></ColdProField><ColdProField label="Condutividade congelado" helpKey="thermalConductivityFrozen" unit="W/m·K"><ColdProInput {...catalogThermalNum("thermal_conductivity_frozen_w_m_k")} /></ColdProField>
            </div></div>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-3">
              <ColdProCalculatedInfo label="Prévia da carga do produto" value={firstThermalLoadReady ? `${fmtColdPro(tunnelResult.productLoadKW * 859.845, 0)} kcal/h` : "Aguardando massa e tempo"} description={`${fmtColdPro(tunnelResult.productLoadKW, 2)} kW`} tone={firstThermalLoadReady ? "success" : "warning"} />
