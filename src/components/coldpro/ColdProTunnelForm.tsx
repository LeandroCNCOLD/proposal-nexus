      ...
      <div className="grid grid-cols-1 gap-x-10 xl:grid-cols-2">
        ...
        <div>
          <div className="mb-3 grid gap-3 sm:grid-cols-2">
            ...
            <ColdProCalculatedInfo label="6. Tempo estimado até o núcleo" value={thermalTimeRequiredMin > 0 ? `${fmtColdPro(thermalTimeRequiredMin, 1)} min` : "—"} description={`para ${fmtColdPro(projectedCycleMassKg, 1)} kg na retenção`} tone={thermalTimeRequiredMin > 0 && thermalTimeRequiredMin <= availableTimeForMassMin ? "success" : "warning"} />
            <ColdProCalculatedInfo label={`7. Retenção necessária para ${fmtColdPro(projectedCycleMassKg, 1)} kg`} value={thermalTimeRequiredMin > 0 ? `${fmtColdPro(thermalTimeRequiredMin, 1)} min` : "—"} description="tempo estimado até o núcleo" tone={thermalTimeRequiredMin > 0 && thermalTimeRequiredMin <= availableTimeForMassMin ? "success" : "warning"} />
            ...
            <ColdProCalculatedInfo label="10. Velocidade ajustada da esteira" value={requiredBeltSpeedMMin > 0 ? `${fmtColdPro(requiredBeltSpeedMMin, 2)} m/min` : "—"} description={beltSpeedVariationPercent === null ? "comprimento útil ÷ tempo necessário" : `${fmtColdPro(beltSpeedVariationPercent, 1)}% vs atual`} tone={requiredBeltSpeedMMin > 0 ? "info" : "warning"} />
            <ColdProCalculatedInfo label="11. Nova capacidade por minuto" value={adjustedCapacityKgMin > 0 ? `${fmtColdPro(adjustedCapacityKgMin, 2)} kg/min` : "—"} description="massa na retenção ÷ retenção necessária" tone={adjustedCapacityKgMin > 0 && adjustedCapacityKgMin >= capacityKgMin ? "success" : "warning"} />
            <ColdProCalculatedInfo label="12. Nova capacidade por hora" value={adjustedCapacityKgH > 0 ? `${fmtColdPro(adjustedCapacityKgH, 1)} kg/h` : "—"} description="nova kg/min × 60" tone={adjustedCapacityKgH > 0 && adjustedCapacityKgH >= nominalCapacityKgH ? "success" : "warning"} />
          ...
          {showInsufficientTimeDiagnostic ? <div className="mb-3 rounded-lg border border-warning/20 bg-warning/10 p-3 text-sm text-warning"><div className="mb-1 flex items-center gap-2 font-semibold"><AlertTriangle className="h-4 w-4" /> Com {fmtColdPro(projectedCycleMassKg, 1)} kg na retenção atual de {fmtColdPro(availableTimeForMassMin, 1)} min, as condições atuais não são compatíveis.</div><p>O modelo estima {fmtColdPro(thermalTimeRequiredMin, 1)} min até o núcleo para essa massa.</p><p className="mt-1">Para manter {fmtColdPro(projectedCycleMassKg, 1)} kg por ciclo, a retenção precisa subir para {fmtColdPro(thermalTimeRequiredMin, 1)} min.</p><p className="mt-1 font-semibold">Isso reduz a velocidade da esteira de {currentBeltSpeedMMin > 0 ? fmtColdPro(currentBeltSpeedMMin, 2) : "—"} para {requiredBeltSpeedMMin > 0 ? fmtColdPro(requiredBeltSpeedMMin, 2) : "—"} m/min. A nova capacidade estimada fica em {adjustedCapacityKgMin > 0 ? fmtColdPro(adjustedCapacityKgMin, 2) : "—"} kg/min, ou {adjustedCapacityKgH > 0 ? fmtColdPro(adjustedCapacityKgH, 1) : "—"} kg/h.</p></div> : null}
        </div>
      </div>
