...
export const ColdProTunnelForm = React.forwardRef<ColdProTunnelFormHandle, ColdProTunnelFormProps>(function ColdProTunnelForm({ environmentId, environment, product, tunnel, productCatalog = [], onSave }, ref) {
  ...
  const adjustedCapacityKgMin = projectedCycleMassKg > 0 && thermalTimeRequiredMin > 0 ? projectedCycleMassKg / thermalTimeRequiredMin : 0;
  const adjustedCapacityKgH = adjustedCapacityKgMin * 60;
      ...
      <div className="grid grid-cols-1 gap-x-10 xl:grid-cols-2">
        ...
        <div>
          <div className="mb-3 grid gap-3 sm:grid-cols-2">
            ...
            <ColdProCalculatedInfo label="10. Velocidade simulada pelo modelo" value={requiredBeltSpeedMMin > 0 ? `${fmtColdPro(requiredBeltSpeedMMin, 2)} m/min` : "—"} description={beltSpeedVariationPercent === null ? "se retenção = tempo térmico" : `simulação: ${fmtColdPro(beltSpeedVariationPercent, 1)}% vs atual`} tone={requiredBeltSpeedMMin > 0 ? "info" : "warning"} />
          ...
          {showInsufficientTimeDiagnostic ? <div className="mb-3 rounded-lg border border-warning/20 bg-warning/10 p-3 text-sm text-warning"><div className="mb-1 flex items-center gap-2 font-semibold"><AlertTriangle className="h-4 w-4" /> Operacionalmente, o ciclo atual é de {fmtColdPro(availableTimeForMassMin, 1)} min para {fmtColdPro(projectedCycleMassKg, 1)} kg.</div><p>A capacidade nominal permanece {fmtColdPro(nominalCapacityKgH, 1)} kg/h, equivalente a {fmtColdPro(capacityKgMin, 2)} kg/min.</p><p className="mt-1">O modelo térmico estimou {fmtColdPro(thermalTimeRequiredMin, 1)} min até o núcleo; isso é uma auditoria física e não deve reduzir automaticamente a velocidade ou a capacidade do girofreezer.</p><p className="mt-1 font-semibold">Se esse tempo térmico fosse aceito como nova retenção, apenas em simulação a velocidade iria de {currentBeltSpeedMMin > 0 ? fmtColdPro(currentBeltSpeedMMin, 2) : "—"} para {requiredBeltSpeedMMin > 0 ? fmtColdPro(requiredBeltSpeedMMin, 2) : "—"} m/min, com {adjustedCapacityKgMin > 0 ? fmtColdPro(adjustedCapacityKgMin, 2) : "—"} kg/min ({adjustedCapacityKgH > 0 ? fmtColdPro(adjustedCapacityKgH, 1) : "—"} kg/h). Revise h, vazão, temperatura, espessura, geometria ou fator de penetração antes de usar essa simulação.</p></div> : null}
        </div>
      </div>
