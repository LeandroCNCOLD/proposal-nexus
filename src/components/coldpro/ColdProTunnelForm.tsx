        ...
        <TabsContent value="continuo">
          <ColdProFormSection title="Contínuo / girofreezer" description="Produto individualizado ou camada fina exposta ao ar." icon={<Wind className="h-4 w-4" />}>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <ColdProCalculatedInfo label="Carga específica" value={`${fmtColdPro(giroResult.thermal.qSpecificTotalKjKg, 2)} kJ/kg`} description="energia total por kg" tone={giroResult.thermal.qSpecificTotalKjKg > 0 ? "success" : "warning"} />
              <ColdProCalculatedInfo label="Sensível acima" value={`${fmtColdPro(giroResult.thermal.qSpecificAboveKjKg, 2)} kJ/kg`} description="Cp acima × ΔT" tone="info" />
              <ColdProCalculatedInfo label="Latente" value={`${fmtColdPro(giroResult.thermal.qSpecificLatentKjKg, 2)} kJ/kg`} description="calor latente × fração" tone="info" />
              <ColdProCalculatedInfo label="Sensível abaixo" value={`${fmtColdPro(giroResult.thermal.qSpecificBelowKjKg, 2)} kJ/kg`} description="Cp abaixo × ΔT" tone="info" />
              <ColdProCalculatedInfo label="Carga do produto" value={`${fmtColdPro(giroResult.thermal.productLoadKw, 2)} kW`} description="kg/h × kJ/kg" tone={giroResult.thermal.productLoadKw > 0 ? "success" : "warning"} />
              <ColdProCalculatedInfo label="Carga da embalagem" value={`${fmtColdPro(giroResult.thermal.packagingLoadKw, 2)} kW`} description="massa embalagem × Cp × ΔT" tone="info" />
              <ColdProCalculatedInfo label="Carga total processo" value={`${fmtColdPro(giroResult.thermal.totalProcessLoadKw, 2)} kW`} description="produto + embalagem" tone={giroResult.thermal.totalProcessLoadKw > 0 ? "success" : "warning"} />
              <ColdProCalculatedInfo label="Capacidade em kcal/h" value={`${fmtColdPro(giroResult.thermal.totalProcessLoadKcalH, 0)} kcal/h`} description="carga total convertida" tone="info" />
              <ColdProCalculatedInfo label="Capacidade em TR" value={`${fmtColdPro(giroResult.thermal.totalProcessLoadTr, 2)} TR`} description="carga total convertida" tone="info" />
              <ColdProCalculatedInfo label="Vazão de ar necessária" value={fmtMaybe(giroResult.thermal.requiredAirflowM3H, 0, " m³/h")} description="potência ÷ ar × Cp × ΔT" tone={giroResult.thermal.requiredAirflowM3H ? "info" : "warning"} />
