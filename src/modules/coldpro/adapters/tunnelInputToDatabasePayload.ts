...
export function tunnelResultToDatabasePayload(form: TunnelSourceRecord, tunnelResult: TunnelEngineResult): TunnelDatabasePayload {
  ...
  const payload = {
    ...
    belt_speed_m_min: round(beltSurface.speedMMin ?? form?.belt_speed_m_min),
    belt_width_m: round(beltSurface.widthM ?? form?.belt_width_m),
    belt_effective_length_m: round(beltSurface.effectiveLengthM ?? form?.belt_effective_length_m),
    belt_surface_density_kg_m2: round(beltSurface.surfaceDensityKgM2 ?? form?.belt_surface_density_kg_m2),
